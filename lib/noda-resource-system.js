  ...
  return {
    ...
    listResourcesSync: function listResources(dir, cb) {
      ...
      if (archive_target == null) {
        ...
        if (entries.length === 0 && !archive_directory[archive_target]) {
          throw new Error('could not find resource');
        }
        return entries;
      }
    ...
    listResources: function listResources(dir, cb) {
      // absolute paths are not allowed
      var archive_target = to_archive_path(dir);
      if (archive_target == null || !archive_directory[archive_target] || !archive_directory[archive_target].isDirectory()) {
        ...
        if (entries.length === 0 && !archive_directory[archive_target]) {
          cb(new Error('could not find resource'));
        }
