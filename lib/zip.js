...
function ZipDirectoryEntry(file, header, entryName, extraField, comment) {
  this.file = file;
  this.header = header;
  if (this.header.readUInt32LE(0) != 0x02014b50) {
...
Object.defineProperties(ZipDirectoryEntry.prototype, {
  toBuffer: {
    value: function (localEntry) {
      var size = 0;
      size += this.header.length;
      size += this.name.length;
      size += this.extraField.length;
...
ZipDirectoryEntry.prototype.entrySync = function () {
  ...
  file.readSync(buff, 0, variable_size, localHeaderOffset + 0x1e);
...
Object.defineProperties(ZipEntry.prototype, {
  toBuffer: {
    value: function () {
      var contents = this.readFileSync();
      var deflated = new Buffer(pako.deflateRaw(contents));
      var usedDeflated = false;
      var size = 0
      size += this.header.length;
      size += this.name.length;
      size += this.extraField.length;
	...
	compression = 8;
      ...
      this.header.copy(buff, 0);
      buff.writeUInt32LE(0x04034b50, 0x00);
      buff.writeUInt16LE(0, 0x04);
      buff.writeUInt16LE(0, 0x06);
      buff.writeUInt32LE(compression, 0x08);
      var checksum = crc32(contents);
      buff.writeUInt32LE(checksum.readUInt32BE(0), 0x0e);
      buff.writeUInt32LE(contents.length, 0x16);
      buff.writeUInt16LE(this.name.length, 0x1a);
      buff.writeUInt16LE(this.extraField.length, 0x1c);
      if (compression === 0) {
	buff.writeUInt32LE(contents.length, 0x12);
        contents.copy(buff, this.header.length);
      }
      else if (compression === 8) {
	buff.writeUInt32LE(deflated.length, 0x12);
        deflated.copy(buff, this.header.length);
      }
      else {
  ...
  nameSize: {
    ...
    set: function (v) {
      return this.header.writeUInt16LE(0x1a, v);
    }
  },
  extraFieldSize: {
    get: function () {
    ...
    set: function (v) {
      return this.header.writeUInt16LE(0x1c, v);
    }
  },
  headerSize: {
    get: function () {
...
// entries = [ {original: Buffer, duplicateEntries: [Buffer], stream} ]
Zip.writeFile = function (filename, entries) {
  ;
}
